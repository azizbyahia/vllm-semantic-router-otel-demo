...
> **A GitHub-rendered copy of the blog post** (images use repo-relative paths so
> they display here). The canonical version for the blog platform lives in
> [`semantic-routing-observability.md`](semantic-routing-observability.md).
>
> *By [Mohamed Aziz Ben Yahia](https://www.linkedin.com/in/mohamed-aziz-ben-yahia/)*
...
![ExtProc request flow](images/sr3.png)
...
> **Want to skip the copy-paste?** Everything below is also packaged as applyable
> files in this repo. Just run [`../scripts/install.sh`](../scripts/install.sh)
> and follow the printed next steps.
...
Use [`../values/otel-collector-values.yaml`](../values/otel-collector-values.yaml). It runs the collector as a DaemonSet, enables the OTLP receivers, and adds an exporter that forwards every trace and log on to Coroot's ingest endpoint (`coroot-coroot.coroot.svc.cluster.local:8080`):
...
Install agentgateway CRDs and the controller via Helm:
...
Apply [`../manifests/gateway.yaml`](../manifests/gateway.yaml):
...
For local testing, the guide uses a lightweight OpenAI-compatible simulator that mimics a vLLM backend serving a base model plus LoRA adapters — see [`../manifests/vllm-sim.yaml`](../manifests/vllm-sim.yaml):

```bash
kubectl apply -f manifests/vllm-sim.yaml
...
```yaml
# values/semantic-router-values.yaml (abridged)
config:
  version: v0.3
...
helm install semantic-router oci://ghcr.io/vllm-project/semantic-router/charts/semantic-router \
  --namespace agentgateway-system \
  --create-namespace \
  --values values/semantic-router-values.yaml
```
...
The **HTTPRoute** connects the gateway to the vLLM backend. Note that no `model` field is set here — the semantic router will inject it. Apply [`../manifests/httproute-backend.yaml`](../manifests/httproute-backend.yaml):

```bash
kubectl apply -f manifests/httproute-backend.yaml
```

The **AgentgatewayPolicy** attaches the semantic router as an ExtProc filter — see [`../manifests/extproc-policy.yaml`](../manifests/extproc-policy.yaml):

```bash
kubectl apply -f manifests/extproc-policy.yaml
```
...
We do this with a second `AgentgatewayPolicy` of type `tracing`, attached to the same gateway ([`../manifests/tracing-policy.yaml`](../manifests/tracing-policy.yaml)). Note the `attributes` block — it lets you enrich each span with request metadata (here: a custom `x-header-tag` header and the request host), which is invaluable for filtering traces by tenant, route, or client.

```bash
kubectl apply -f manifests/tracing-policy.yaml
```
...
You should see `"math-expert"` and `"science-expert"` in the responses respectively — the semantic router correctly classified and mutated each request. (This is exactly what [`../scripts/test.sh`](../scripts/test.sh) automates.)
...
Send a batch of requests to generate trace data ([`../scripts/load.sh`](../scripts/load.sh)):
...
Each trace for a routing decision shows a hierarchy of spans. Because we enabled gateway-level tracing in Step 8, the trace now starts at the **agentgateway frontend span** and continues down into the router — one continuous timeline from the moment the request hits the gateway to the mutated response:
